...
export class TaxRemittanceService {
      ...
      const journal = await this.ledger.postJournal(
        {
          ...
          lines: [
            { accountId: taxRate.glAccountId, debit: dto.amount, projectId: dto.projectId },
            { accountId: bankAccount.glAccountId, credit: dto.amount, projectId: dto.projectId },
          ],
        },
        tx,
      );
  ...
  findAll(params: { skip?: number; take?: number; taxRateId?: string; projectId?: string }) {
    const { skip, take, taxRateId, projectId } = params;
    return this.prisma.taxRemittance.findMany({
      where: {
        ...(taxRateId ? { taxRateId } : {}),
        ...(projectId ? { projectId } : {}),
      },
      include: { taxRate: true, bankAccount: true, project: true },
