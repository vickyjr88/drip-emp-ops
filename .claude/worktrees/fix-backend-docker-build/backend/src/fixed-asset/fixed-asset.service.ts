...
export class FixedAssetService {
  ...
  findAll(params: { skip?: number; take?: number; projectId?: string; status?: string }) {
    const { skip, take, projectId, status } = params;
  ...
  async runDepreciation(periodStartInput: string) {
    ...
    for (const asset of assets) {
      ...
      const journal = await this.ledger.postJournal({
        ...
        sourceId: asset.id,
        lines: [
          { accountId: depreciationExpenseAccount.id, debit: monthly, projectId: asset.projectId || undefined },
          { accountId: accumulatedDepreciationAccount.id, credit: monthly, projectId: asset.projectId || undefined },
        ],
      });
