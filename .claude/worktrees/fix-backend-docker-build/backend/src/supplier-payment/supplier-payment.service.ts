...
export class SupplierPaymentService {
  ...
  async release(id: string) {
    ...
    const projectIds = new Set(
      payment.allocations.map((allocation) => allocation.supplierInvoice.projectId).filter((id): id is string => !!id),
    );
    ...
    const lines = [
      { accountId: apAccount.id, debit: Number(payment.amount), currencyCode: payment.currency, projectId },
      { accountId: bankAccount.glAccountId, credit: netCashOut, currencyCode: payment.currency, projectId },
    ];
    if (whtAmount > 0 && payment.whtRateId) {
      const whtRate = await this.prisma.taxRate.findUnique({ where: { id: payment.whtRateId } });
      if (whtRate) {
        lines.push({ accountId: whtRate.glAccountId, credit: whtAmount, currencyCode: payment.currency, projectId });
      }
    }
