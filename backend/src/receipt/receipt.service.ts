  ...
  async create(dto: CreateReceiptDto) {
    ...
    if (allocations.length > 0) {
      ...
      if (firstInvoice) {
        ...
        storeId = firstInvoice.storeId ?? undefined;
      }
    }

    const bankAccount = await this.accountResolver.resolveBankAccount(purpose, storeId, dto.bankAccountId);
      ...
      const journal = await this.ledger.postJournal(
        {
          ...
          sourceId: receipt.id,
          lines: [
            { accountId: bankAccount.glAccountId, debit: dto.amount, currencyCode: currency, storeId },
            { accountId: arAccount.id, credit: dto.amount, currencyCode: currency, storeId },
          ],
        },
        tx,
      );
