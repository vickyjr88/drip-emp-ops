...
function makeForm(project?: Project | null): ProjectFormState {
  ...
  return {
    ...
    yearBuilt: project?.yearBuilt != null ? String(project.yearBuilt) : '',
    ...
    principalAgentId: project?.principalAgentId || '',
  };
}
...
export default function ProjectEditClient({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async (authToken: string) => {
    ...
    try {
      ...
      // The layouts on offer in this development. Read-only here -- the plan
      // form owns editing -- so a missing permission just hides the section.
      if (hasPermission(nextProfile, 'floor-plan.read')) {
        try {
          setFloorPlans(
            await apiRequest<typeof floorPlans>(
              `/floor-plans?projectId=${projectId}`,
              { method: 'GET' },
              authToken,
            ),
          );
      ...
      // Populates the principal agent picker; a missing user.read permission
      // just leaves the list empty rather than failing the whole page.
      try {
      ...
      await apiRequest(
        `/projects/${projectId}`,
        {
          ...
          body: JSON.stringify({
            ...
            exteriorMaterials: form.exteriorMaterials || undefined,
            addressLine: form.addressLine || undefined,
            city: form.city || undefined,
            country: form.country || undefined,
            featuredOrder: form.featuredOrder ?? undefined,
            principalAgentId: form.principalAgentId || null,
          }),
        },
        token,
      );
        ...
        <section className="lp-container portal-auth-section">
          ...
          >
            ...
            <form className="portal-stack-grid" onSubmit={onSubmit}>
              <article className="portal-card">
                ...
                <div className="portal-entity-form">
                  ...
                  <label>
                    ...
                    <select
                      value={form.principalAgentId}
                      onChange={(event) => setForm((prev) => ({ ...prev, principalAgentId: event.target.value }))}
                    >
                  ...
                  <div className="portal-entity-grid-3">
                    <label>
                      ...
                      <input
                        ...
                        placeholder="2024"
                        onChange={(event) => setForm((prev) => ({ ...prev, yearBuilt: event.target.value }))}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        placeholder="High-rise residential tower"
                        onChange={(event) => setForm((prev) => ({ ...prev, structureType: event.target.value }))}
                      />
                    </label>
                    <label>
                      <span>Exterior materials</span>
                      <input
                        value={form.exteriorMaterials ?? ''}
                        placeholder="Glass, aluminium and textured render"
                        onChange={(event) => setForm((prev) => ({ ...prev, exteriorMaterials: event.target.value }))}
                      />
                    </label>
                  </div>
              ...
              {hasPermission(profile, 'floor-plan.read') ? (
                <article className="portal-card">
                  <div className="portal-card-header-row">
                    <div>
                      <h2 style={{ margin: 0 }}>Floor Plans</h2>
                      <p className="portal-muted">
                        The layouts offered in this development. Units are created from a plan and
                        copy its figures at that moment — editing a plan never changes units already
                        built from it.
                      </p>
                    </div>
                    {hasPermission(profile, 'floor-plan.create') ? (
                      <Link
                        href={`/portal/floor-plans/new?projectId=${projectId}`}
                  ...
                  <div className="portal-list-stack">
                    {floorPlans.length === 0 ? (
                      <div className="portal-empty-state">
                        No floor plans yet. Add one so units in this project can be created from it.
                      </div>
                    ) : (
                      floorPlans.map((plan) => (
                        <div key={plan.id} className="portal-record">
                          <div className="portal-list-row">
                            <div>
                              ...
                              <p className="portal-muted">
                                {plan._count?.units ?? 0} unit
                                {(plan._count?.units ?? 0) === 1 ? '' : 's'} created from this plan
                              </p>
                            </div>
                            <div className="portal-action-row">
                              {hasPermission(profile, 'floor-plan.update') ? (
                                <Link
                                  href={`/portal/floor-plans/${plan.id}/edit`}
